...
async function ensureSchema(client: NeonQueryFunction<false, false>) {
  ready ??= (async () => {
    ...
    await client`CREATE TABLE IF NOT EXISTS cells (id text PRIMARY KEY, rack_id text NOT NULL REFERENCES racks(id) ON DELETE CASCADE, code text NOT NULL UNIQUE, label text NOT NULL, row_index integer NOT NULL, column_index integer NOT NULL, blocked boolean NOT NULL DEFAULT false)`;
