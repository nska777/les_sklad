      ...
      <Dialog open={!!selectedCell} onOpenChange={(open) => { if (!open) { setSelectedCell(null); setCellMode("overview"); resetCellForms(); } }}>
        <DialogContent className="max-h-[94vh] overflow-y-auto sm:max-w-3xl">
          {selectedCell && <>
            ...
            {cellMode === "overview" && <section className="mt-4"><div className="mb-2 text-xs font-semibold uppercase tracking-widest text-slate-500">Содержимое ячейки</div>{selectedStocks.length ? <div className="space-y-2">{selectedStocks.map((stock) => <div key={stock.productId} className="rounded-xl border bg-slate-50 p-4"><div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_220px_auto] sm:items-center"><div className="flex min-w-0 items-start gap-3"><Boxes size={18} className="mt-0.5 shrink-0 text-blue-600" /><div className="min-w-0"><div className="font-semibold">{stock.productName}</div><div className="mt-1 text-xs text-slate-500">RL-код</div><div className="font-mono text-sm font-semibold text-slate-700">{stock.sku}</div></div></div><ProductBarcode value={stock.barcode || stock.sku} compact className="w-full" /><b className="text-base sm:text-right">{qty(stock.quantity)} {stock.unit}</b></div></div>)}</div> : <div className="rounded-xl bg-slate-50 p-5 text-sm text-slate-500">Ячейка свободна. Нажмите «Добавить», чтобы разместить материал.</div>}</section>}
