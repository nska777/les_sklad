...
export default function RackLayoutPage() {
  ...
  const showSide = (nextSide: "front" | "back") => {
    if (nextSide === "back" && !hasBack) return;
    setSide(nextSide);
    setRotationY(nextSide === "front" ? 0 : 180);
  };

  const onRackPointerDown = (event: ReactPointerEvent<HTMLDivElement>) => {
    if (!hasBack) return;
  ...
  const onRackPointerMove = (event: ReactPointerEvent<HTMLDivElement>) => {
    if (!dragging || !hasBack) return;
    const delta = event.clientX - dragStartX.current;
    setRotationY(dragStartRotation.current + delta * 0.45);
  };
  ...
  const finishRackDrag = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const backVisible = angle > 90 && angle < 270;
    const nextSide = backVisible && hasBack ? "back" : "front";
    setSide(nextSide);
  ...
  const createRack = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setCreateOpen(false); setSide("front"); setRotationY(0); toast.success("Стеллаж создан");
  ...
  const updateRack = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await load(); setEditRack(null); setSide("front"); setRotationY(0); toast.success("Стеллаж обновлён");
  ...
  const deleteRack = async () => {
    ...
    setSaving(true);
    try { await apiAction({ action: "deleteRack", rackId: activeRack.id, operator: "Кладовщик" }); await load(); setSide("front"); setRotationY(0); toast.success("Стеллаж удалён"); }
  ...
  const enableBack = async () => {
    ...
    setSaving(true);
    try { await apiAction({ action: "enableBackSide", rackId: activeRack.id, operator: "Кладовщик" }); await load(); setSide("back"); setRotationY(180); toast.success("Задняя сторона создана"); }
  ...
  return <main className="min-h-screen px-3 py-4 text-[var(--foreground)] sm:px-5 lg:px-7">
    <div className="mx-auto max-w-[1700px] space-y-4">
      ...
      {loading ? <div className="panel flex min-h-[460px] items-center justify-center"><Loader2 className="animate-spin" /></div> : !activeRack ? <div className="panel p-12 text-center text-slate-500">Стеллажей пока нет</div> : <section className="panel overflow-hidden">
        <div className="flex flex-col gap-3 border-b border-black/10 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5"><div><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[.14em] text-slate-500"><Layers3 size={15} /> Стеллаж {activeRack.code}</div><h2 className="mt-1 text-2xl font-bold">{activeRack.name}</h2></div><div className="flex flex-wrap items-center gap-2"><div className="rounded-xl border border-black/10 bg-slate-100 p-1"><button type="button" onClick={() => showSide("front")} className={`cursor-pointer rounded-lg px-4 py-2 text-sm font-semibold ${side === "front" ? "bg-white text-blue-700 shadow" : "text-slate-500"}`}>Лицевая</button><button type="button" disabled={!hasBack} onClick={() => showSide("back")} className={`cursor-pointer rounded-lg px-4 py-2 text-sm font-semibold ${side === "back" ? "bg-white text-blue-700 shadow" : "text-slate-500 disabled:opacity-40"}`}>Задняя</button></div>{!hasBack && <Button disabled={saving} variant="outline" onClick={() => void enableBack()}><Plus /> Задняя сторона</Button>}<Button variant="outline" onClick={() => setEditRack(activeRack)}><Pencil /> Изменить</Button><Button variant="outline" className="text-red-600" onClick={() => void deleteRack()}><Trash2 /> Удалить</Button></div></div>

        <div className="overflow-x-auto p-3 sm:p-6">
          <div className="mx-auto min-w-[720px] max-w-[1350px] select-none [perspective:1800px]">
            <div className="mb-3 flex items-center justify-center gap-2 text-xs font-medium text-slate-500"><Rotate3D size={15}/>{hasBack ? "Перетащите стеллаж мышкой, чтобы повернуть его" : "Односторонний стеллаж"}</div>
            <div
              className={`relative cursor-${hasBack ? "grab" : "default"} touch-none transition-[transform] ${dragging ? "duration-0 cursor-grabbing" : "duration-500"} [transform-style:preserve-3d]`}
              style={{ transform: `rotateX(1deg) rotateY(${rotationY}deg)` }}
              onPointerDown={onRackPointerDown}
              onPointerMove={onRackPointerMove}
              onPointerUp={finishRackDrag}
              onPointerCancel={finishRackDrag}
            >
              <RackFace rack={activeRack} side="front" cells={rackCells} stocks={data.stocks} shelves={shelves} onCell={setSelectedCell} className="relative [backface-visibility:hidden] [transform:translateZ(36px)]" />
              {hasBack && <RackFace rack={activeRack} side="back" cells={rackCells} stocks={data.stocks} shelves={shelves} onCell={setSelectedCell} className="absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)_translateZ(36px)]" />}
            </div>
          </div>
        </div>
      </section>}
...
function RackFace({ rack, side, cells, stocks, shelves, onCell, className }:{ rack: Rack; side:"front"|"back"; cells:Cell[]; stocks:Stock[]; shelves:number[]; onCell:(cell:Cell)=>void; className?:string }) {
  const sideCells = cells.filter((cell)=>cell.side===side);
  return <div className={`${className || ""} rounded-[26px] border-[10px] border-slate-800 bg-slate-900 p-3 shadow-2xl`}>
    <div className="mb-3 flex items-center justify-center rounded-xl bg-gradient-to-b from-slate-700 to-slate-900 py-3 font-bold tracking-[.2em] text-white">{rack.code} · {side === "front" ? "ЛИЦЕВАЯ" : "ЗАДНЯЯ"} СТОРОНА</div>
    <div className="space-y-3">{shelves.map((rowIndex)=>{ const rowCells=sideCells.filter((cell)=>cell.rowIndex===rowIndex).sort((a,b)=>a.columnIndex-b.columnIndex); return <div key={rowIndex} className="grid grid-cols-[70px_1fr] gap-2 rounded-xl bg-slate-800 p-2"><div className="flex flex-col items-center justify-center rounded-lg bg-slate-700 text-white"><span className="text-[10px] uppercase tracking-widest text-slate-300">Полка</span><b className="text-2xl">{rowIndex+1}</b></div><div className="grid gap-2" style={{gridTemplateColumns:`repeat(${rack.columns},minmax(0,1fr))`}}>{Array.from({length:rack.columns},(_,columnIndex)=>{const cell=rowCells.find((item)=>item.columnIndex===columnIndex); if(!cell)return <div key={columnIndex} className="min-h-[125px] rounded-xl border border-dashed border-slate-600 bg-slate-900/60"/>; const cellStocks=stocks.filter((stock)=>stock.cellId===cell.id); return <button key={cell.id} onPointerDown={(event)=>event.stopPropagation()} onClick={()=>onCell(cell)} className={`min-h-[125px] cursor-pointer rounded-xl border p-3 text-left transition hover:-translate-y-0.5 hover:shadow-xl ${cellStocks.length?"border-blue-300 bg-gradient-to-br from-blue-50 to-indigo-100":"border-slate-200 bg-gradient-to-br from-white to-slate-100"}`}><div className="flex items-start justify-between"><div><div className="text-[10px] font-semibold uppercase tracking-[.12em] text-slate-400">Ячейка</div><b className="font-mono text-sm">{cell.code}</b></div><QrCode size={15}/></div>{cellStocks.length?<div className="mt-4 space-y-2">{cellStocks.slice(0,2).map((stock)=><div key={stock.productId} className="rounded-lg bg-white/80 px-2 py-1.5"><div className="line-clamp-1 text-xs font-semibold">{stock.productName}</div><div className="text-xs font-bold text-blue-700">{qty(stock.quantity)} {stock.unit}</div></div>)}</div>:<div className="mt-7 text-xs font-medium text-slate-400">Свободно</div>}</button>})}</div></div>})}</div>
