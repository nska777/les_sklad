...
const numberValue = (v: unknown) => { if (typeof v === "number") return Number.isFinite(v) ? v : 0; const n = Number(String(v ?? "").trim().replace(/\s/g, "").replace(",", ".")); return Number.isFinite(n) ? n : 0; };
...
export default function DepartmentOneCMaterialsPage() {
  ...
  const factByCatalog = useMemo(() => {
    ...
    data.products.forEach((item) => {
      ...
      map.set(item.id, data.stocks.filter((s) => s.productId === item.linkedProductId).reduce((sum, s) => sum + Number(s.quantity), 0));
    });
    return map;
  ...
  const locationsByCatalog = useMemo(() => {
    ...
    data.products.forEach((item) => {
      if (!item.linkedProductId) return;
      const labels = data.stocks.filter((s) => s.productId === item.linkedProductId && Number(s.quantity) > 0).map((s) => {
        const cell = cellMap.get(s.cellId); const rack = cell ? rackMap.get(cell.rackId) : undefined;
        return cell && rack ? `${cell.code}: ${fmt(Number(s.quantity))} ${item.unit}` : "";
      }).filter(Boolean);
  ...
  const openPlace = (p: CatalogItem) => { setPlacing(p); setRackId(data.racks[0]?.id || ""); setCellId(""); const remaining = Math.max(0, Number(p.quantity1c || 0) - Number(factByCatalog.get(p.id) || 0)); setQuantity(String(remaining > 0 ? remaining : 1)); };
  ...
  const place = async () => {
    ...
    setSaving(true);
    try { const r = await fetch("/api/department-onec", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "place", catalogId: placing.id, cellId, quantity }) }); const body = await r.json() as { error?: string }; if (!r.ok) throw new Error(body.error || "Не удалось разместить материал"); toast.success("Фактический материал создан и размещён"); setPlacing(null); await load(); }
  ...
  return <main className="min-h-screen px-4 py-5 text-[var(--foreground)] sm:px-7"><div className="mx-auto max-w-[1650px] space-y-4">
    ...
    <section className="panel overflow-hidden p-0"><div className="flex flex-wrap items-center justify-between gap-3 border-b p-4"><div className="relative w-full max-w-2xl"><Search size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Поиск: наименование, артикул, штрихкод, ID 1С..." className="h-12 w-full rounded-xl border pl-10 pr-3 outline-none focus:border-blue-500"/></div><div className="text-sm text-slate-400">Найдено: {filtered.length}</div></div>
      {loading ? <div className="flex h-60 items-center justify-center"><Loader2 className="animate-spin"/></div> : <div className="divide-y">{shown.map((p) => { const fact = Number(factByCatalog.get(p.id) || 0); const remaining = Math.max(0, Number(p.quantity1c || 0) - fact); const locations = locationsByCatalog.get(p.id) || []; return <div key={p.id} className="grid gap-3 p-4 xl:grid-cols-[1.7fr_.55fr_.55fr_1fr_1.15fr_auto] xl:items-center"><div><div className="font-black">{p.name}</div><div className="mt-1 text-xs text-slate-400">{p.sku}{p.oneCId ? ` · ID 1С: ${p.oneCId}` : ""}</div></div><div><div className="text-xs text-slate-400">По 1С</div><b>{fmt(p.quantity1c)} {p.unit}</b></div><div><div className="text-xs text-slate-400">Факт</div><b>{fmt(fact)} {p.unit}</b><div className="text-xs text-blue-600">Осталось: {fmt(remaining)}</div></div><div><div className="text-xs text-slate-400">Где лежит</div><div className="text-sm">{locations.length ? locations.join(" · ") : "Не размещён"}</div></div><div><div className="text-xs text-slate-400">Штрихкод материала</div><div className="mt-1 inline-block rounded-lg bg-white px-2 py-1">{p.barcode ? <ReactBarcode value={p.barcode} height={28} width={1.15} displayValue={true} fontSize={10} margin={0}/> : <span className="text-xs text-slate-400">Будет создан при размещении</span>}</div></div><button onClick={() => openPlace(p)} className="rounded-xl bg-orange-500 px-4 py-2 font-bold text-white">Разместить</button></div>; })}{!shown.length && <div className="p-12 text-center text-slate-400">Ничего не найдено</div>}</div>}
      {pages > 1 && <div className="flex items-center justify-center gap-3 border-t p-4"><button disabled={page <= 1} onClick={() => setPage((x) => Math.max(1, x - 1))} className="rounded-lg border px-3 py-1.5 disabled:opacity-40">Назад</button><span className="text-sm">{page} / {pages}</span><button disabled={page >= pages} onClick={() => setPage((x) => Math.min(pages, x + 1))} className="rounded-lg border px-3 py-1.5 disabled:opacity-40">Далее</button></div>}
    </section>
  </div>

  {placing && <div className="fixed inset-0 z-[50000] flex items-center justify-center bg-slate-950/55 p-3 backdrop-blur-sm" onMouseDown={(e) => { if (e.target === e.currentTarget) setPlacing(null); }}><div className="w-full max-w-[680px] rounded-[28px] bg-white p-6 shadow-2xl"><div className="flex justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-[.14em] text-orange-600">Фактическое размещение</div><h2 className="mt-1 text-xl font-black">{placing.name}</h2><p className="mt-1 text-sm text-slate-500">После сохранения материал появится в разделе «Материалы» с этим же штрихкодом.</p></div><button onClick={() => setPlacing(null)}><X/></button></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm font-bold">Место хранения<select value={rackId} onChange={(e) => { setRackId(e.target.value); setCellId(""); }} className="mt-1 h-11 w-full rounded-xl border bg-white px-3"><option value="">Выберите...</option>{data.racks.map((r) => <option key={r.id} value={r.id}>{r.storageType === "floor" ? "Напольная зона" : "Стеллаж"} {r.code} · {r.name}</option>)}</select></label><label className="text-sm font-bold">Ячейка<select value={cellId} onChange={(e) => setCellId(e.target.value)} className="mt-1 h-11 w-full rounded-xl border bg-white px-3"><option value="">Выберите...</option>{rackCells.map((c) => <option key={c.id} value={c.id}>{c.code}</option>)}</select></label></div><label className="mt-4 block text-sm font-bold">Фактическое количество ({placing.unit})<input value={quantity} onChange={(e) => setQuantity(e.target.value)} type="number" step="0.001" min="0.001" className="mt-1 h-11 w-full rounded-xl border px-3"/></label><div className="mt-4 rounded-xl bg-slate-50 p-3 text-sm"><b>Штрихкод:</b> {placing.barcode || "будет создан автоматически"}</div><button disabled={saving || !rackId || !cellId} onClick={() => void place()} className="mt-5 h-12 w-full rounded-xl bg-orange-500 font-black text-white disabled:opacity-50">{saving ? "Размещение..." : "Создать фактический материал и разместить"}</button></div></div>}

  {importOpen && <div className="fixed inset-0 z-[50000] flex items-center justify-center bg-slate-950/55 p-3 backdrop-blur-sm" onMouseDown={(e) => { if (e.target === e.currentTarget) setImportOpen(false); }}><div className="max-h-[92vh] w-full max-w-[850px] overflow-auto rounded-[28px] bg-white p-6 shadow-2xl"><div className="flex justify-between"><div><h2 className="text-2xl font-black">Загрузить справочник 1С</h2><p className="mt-1 text-sm text-slate-500">Импорт не создаёт материалы на физическом складе. Он только обновляет справочник 1С.</p></div><button onClick={() => setImportOpen(false)}><X/></button></div><label className="mt-5 flex cursor-pointer items-center justify-center rounded-2xl border-2 border-dashed p-8"><input type="file" accept=".xlsx,.xls,.csv" className="hidden" onChange={async (e) => { const file = e.target.files?.[0]; if (!file) return; try { const parsed = await parseExcel(file); setFileName(file.name); setImportItems(parsed.items); setQtyLabel(parsed.qtyLabel); setBarcodeLabel(parsed.barcodeLabel); } catch (error) { toast.error(error instanceof Error ? error.message : "Не удалось прочитать файл"); } }}/><span className="text-center"><Upload className="mx-auto mb-2"/><b>{fileName || "Выберите Excel / CSV"}</b></span></label>{importItems.length > 0 && <div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-slate-50 p-3"><div className="text-xs text-slate-400">Позиций</div><b>{importItems.length}</b></div><div className="rounded-xl bg-slate-50 p-3"><div className="text-xs text-slate-400">Колонка остатка</div><b>{qtyLabel}</b></div><div className="rounded-xl bg-slate-50 p-3"><div className="text-xs text-slate-400">Штрихкод</div><b>{barcodeLabel}</b></div></div>}<button disabled={saving || !importItems.length} onClick={() => void doImport()} className="mt-5 h-12 w-full rounded-xl bg-orange-500 font-black text-white disabled:opacity-50">{saving ? "Загрузка..." : "Загрузить только в справочник 1С"}</button></div></div>}
  </main>;
}
