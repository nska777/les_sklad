    ...
    <div className="mx-auto max-w-[1800px] space-y-4">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" onClick={() => setMode("floor")}><MapPin /> Напольная зона</Button>
        </div>
      </div>
          ...
          {selected ? <>
            ...
            <div className="mt-5 space-y-2">
              {selectedStocks.map((stock) => {
                const product = data.products.find((x) => x.id === stock.productId);
                return <div key={stock.productId} className="rounded-xl border bg-slate-50 p-3"><div className="font-bold">{product?.name}</div><div className="mt-1 text-sm text-slate-500">{fmt(Number(stock.quantity))} {product?.unit}{product?.packSize ? ` · ≈ ${fmt(Number(stock.quantity) / Number(product.packSize))} ${product.packType || "уп."}` : ""}</div></div>;
              })}
              {!selectedStocks.length && <div className="rounded-xl border border-dashed p-4 text-center text-sm text-slate-400">Место свободно</div>}
            </div>
            <form onSubmit={place} className="mt-5 space-y-3 border-t pt-5">
              <div className="font-black">Разместить материал</div>
              <div><Label>Материал</Label><select name="productId" required className="mt-1.5 h-10 w-full rounded-md border bg-white px-3 text-sm"><option value="">Выберите...</option>{data.products.map((p) => <option key={p.id} value={p.id}>{p.name} · {p.unit}</option>)}</select></div>
      ...
      {mode && <div className="fixed inset-0 z-[12000] flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm">
        <form onSubmit={submitStorage} className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
          <div className="flex items-center gap-3"><div className="rounded-xl bg-slate-100 p-2"><Layers3 size={20} /></div><div><h2 className="text-xl font-black">{mode === "floor" ? "Новая напольная зона" : "Новый стеллаж"}</h2><p className="text-xs text-slate-500">Объект сразу появится в 3D-комнате</p></div></div>
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <div><Label>Название</Label><Input name="name" required className="mt-1.5" /></div>
            <div><Label>Код</Label><Input name="code" required className="mt-1.5" placeholder={mode === "floor" ? "P-01" : "K1"} /></div>
            {mode === "rack" && <><div><Label>Полок</Label><Input name="rows" type="number" min="1" max="12" defaultValue="4" className="mt-1.5" /></div><div><Label>Ячеек на полке</Label><Input name="columns" type="number" min="1" max="20" defaultValue="4" className="mt-1.5" /></div></>}
            <div><Label>Ширина, м</Label><Input name="width" type="number" step="0.1" defaultValue={mode === "floor" ? 2 : 4} className="mt-1.5" /></div>
            <div><Label>Глубина, м</Label><Input name="depth" type="number" step="0.1" defaultValue={mode === "floor" ? 2 : 1.2} className="mt-1.5" /></div>
          </div>
          <div className="mt-5 flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setMode(null)}>Отмена</Button><Button className="accent-button" disabled={saving}><Plus /> Создать</Button></div>
        </form>
      </div>}
    </div>
  </main>;
