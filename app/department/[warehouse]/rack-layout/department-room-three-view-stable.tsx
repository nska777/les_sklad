...
type Movement = { id: string; type: string; productId: string; productName: string; quantity: number; fromCellId?: string | null; toCellId?: string | null; fromCellCode?: string; toCellCode?: string; operator?: string; createdAt: string; comment?: string };
...
export function DepartmentRoomThreeView({ racks, cells, stocks, selectedCellId, selectedRackId, onCellClick, onRackClick, onClearSelection }: {
  ...
  const openCell = useCallback((cell: Cell) => {
    onCellClick(cell); setActiveCell(cell); setMode("contents"); setSearch(""); setProductId(""); setTargetCellId(""); setQty("");
  ...
  useEffect(() => {
    ...
    const camera = new THREE.PerspectiveCamera(45, 1, .1, 120); camera.position.set(cx + 11, 8, cz + 15); camera.lookAt(center);
    const renderer = new THREE.WebGLRenderer({ antialias: true, powerPreference: "low-power" });
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5)); renderer.shadowMap.enabled = false; renderer.outputColorSpace = THREE.SRGBColorSpace;
    renderer.domElement.style.width = "100%"; renderer.domElement.style.height = "100%"; host.appendChild(renderer.domElement);
    const controls = new OrbitControls(camera, renderer.domElement); controls.enableDamping = false; controls.target.copy(center); controls.maxPolarAngle = Math.PI / 2.02;
    ...
    rackOnly.forEach((rack, ri) => {
      ...
      const rackWidth = Math.max(1, Number(rack.width || 4)); const rackDepth = Math.max(.7, Number(rack.depth || 1.2));
      let y = .16;
      ...
      byRows.forEach((row, rowIndex) => {
        ...
        row.forEach((cell, ci) => {
          ...
          const hit = new THREE.Mesh(new THREE.BoxGeometry(Math.max(.18, w - .06), Math.max(.25, h - .08), .14), new THREE.MeshStandardMaterial({ color: cell.id === selectedCellId ? 0xf59e0b : ss.length ? 0xbfdbfe : 0xffffff, transparent: true, opacity: cell.id === selectedCellId ? .7 : ss.length ? .35 : .08 }));
          ...
          const visible = ss.slice(0, 16); const cols = Math.max(1, Math.ceil(Math.sqrt(visible.length || 1)));
          visible.forEach((s, index) => { const mesh = packageMesh(s, Math.min(.9, Math.max(.45, w / Math.max(2, cols)))); const col = index % cols, row = Math.floor(index / cols); mesh.position.set(x + (col - (cols - 1) / 2) * Math.min(.36, w / Math.max(2, cols)), y + .2, -.08 + row * .28); g.add(mesh); });
        });
      ...
      if (rack.id === selectedRackId) scene.add(new THREE.BoxHelper(g, 0xf97316));
    });
    ...
    floorOnly.forEach((rack, i) => {
      ...
      const pad = new THREE.Mesh(new THREE.BoxGeometry(w, .12, d), new THREE.MeshStandardMaterial({ color: rack.id === selectedRackId ? 0xf59e0b : 0xfef3c7, transparent: true, opacity: .75 })); pad.position.set(p.x, .06, p.z); scene.add(pad); cellObjects.push(pad); cellByUuid.set(pad.uuid, cell); rackObjects.push(pad); rackByUuid.set(pad.uuid, rack);
      const ss = stocks.filter((s) => s.cellId === cell.id && Number(s.quantity) > 0).slice(0, 24); const cols = Math.max(1, Math.ceil(Math.sqrt(ss.length || 1)));
    ...
    const resize = () => { const w = Math.max(320, host.clientWidth), h = Math.max(540, host.clientHeight); camera.aspect = w / h; camera.updateProjectionMatrix(); renderer.setSize(w, h, false); render(); };
    ...
    return () => {
      ro.disconnect(); controls.removeEventListener("change", render); controls.dispose(); renderer.domElement.removeEventListener("pointerdown", down); renderer.domElement.removeEventListener("pointerup", up);
  ...
  return <>
    ...
    {activeCell && <div className="fixed inset-0 z-[30000] flex items-center justify-center bg-slate-950/55 p-3 backdrop-blur-sm" onMouseDown={(e) => { if (e.target === e.currentTarget) setActiveCell(null); }}><div className="max-h-[92vh] w-full max-w-[920px] overflow-auto rounded-[28px] bg-white p-5 shadow-2xl sm:p-6"><div className="flex items-start justify-between"><div><div className="text-xs font-black uppercase tracking-[.14em] text-blue-600">Ячейка</div><h2 className="mt-1 text-2xl font-black">{activeCell.code}</h2><p className="mt-1 text-sm text-slate-500">{activeStocks.length} позиций · {activeStocks.length ? "Занята" : "Свободна"}</p></div><button onClick={() => setActiveCell(null)} className="rounded-xl p-2 hover:bg-slate-100"><X/></button></div><div className="mt-5 grid gap-4 md:grid-cols-[170px_1fr]"><div className="text-center"><div className="flex h-[170px] items-center justify-center rounded-2xl border"><QRCodeSVG value={`RL-CELL:${snapshot?.warehouse?.code || "paint"}:${activeCell.code}`} size={140}/></div><div className="mt-2 text-sm font-black">{activeCell.code}</div></div><div className="flex flex-wrap content-start gap-2">{modeButton("contents", "Содержимое", <PackagePlus size={15}/>)}{modeButton("add", "Добавить", <PackagePlus size={15}/>)}{modeButton("move", "Переместить", <ArrowRightLeft size={15}/>)}{modeButton("adjust", "Корректировать", <SlidersHorizontal size={15}/>)}{modeButton("history", "История", <History size={15}/>)}</div></div>
      {mode === "contents" && <div className="mt-5 space-y-3">{activeStocks.map((s) => { const p = productMap.get(s.productId); return <div key={s.productId} className="grid gap-2 rounded-2xl border bg-slate-50 p-4 sm:grid-cols-[1fr_auto] sm:items-center"><div><div className="font-black">{p?.name || "Материал"}</div><div className="mt-1 text-xs text-slate-500">{p?.sku || ""}{p?.oneCId ? ` · 1С ${p.oneCId}` : ""}</div><div className="mt-1 font-mono text-xs text-slate-400">Штрихкод: {p?.barcode || "—"}</div></div><div className="text-lg font-black">{Number(s.quantity).toLocaleString("ru-RU")} {p?.unit || ""}</div></div>; })}{!activeStocks.length && <div className="rounded-2xl border border-dashed p-10 text-center text-slate-400">Ячейка свободна</div>}</div>}
      {mode === "add" && <form onSubmit={add} className="mt-5 space-y-3"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Поиск материала..." className="h-11 w-full rounded-xl border px-3"/><select value={productId} onChange={(e) => setProductId(e.target.value)} className="h-11 w-full rounded-xl border bg-white px-3"><option value="">Выберите материал...</option>{filteredProducts.map((p) => <option key={p.id} value={p.id}>{p.name} · {p.unit}</option>)}</select><input name="quantity" type="number" step="0.001" min="0.001" required placeholder="Количество" className="h-11 w-full rounded-xl border px-3"/><button disabled={busy} className="h-11 w-full rounded-xl bg-orange-500 font-black text-white">Добавить в {activeCell.code}</button></form>}
      {mode === "move" && <form onSubmit={move} className="mt-5 space-y-3"><select value={productId} onChange={(e) => setProductId(e.target.value)} className="h-11 w-full rounded-xl border bg-white px-3"><option value="">Материал из ячейки...</option>{activeStocks.map((s) => { const p = productMap.get(s.productId); return <option key={s.productId} value={s.productId}>{p?.name} · {s.quantity} {p?.unit}</option>; })}</select><select value={targetCellId} onChange={(e) => setTargetCellId(e.target.value)} className="h-11 w-full rounded-xl border bg-white px-3"><option value="">Куда переместить...</option>{(snapshot?.cells || []).filter((c) => c.id !== activeCell.id && !c.blocked).map((c) => <option key={c.id} value={c.id}>{c.code}</option>)}</select><input value={qty} onChange={(e) => setQty(e.target.value)} type="number" step="0.001" min="0.001" required placeholder="Количество" className="h-11 w-full rounded-xl border px-3"/><button disabled={busy} className="h-11 w-full rounded-xl bg-blue-600 font-black text-white">Переместить</button></form>}
      {mode === "adjust" && <form onSubmit={adjust} className="mt-5 space-y-3"><select value={productId} onChange={(e) => setProductId(e.target.value)} className="h-11 w-full rounded-xl border bg-white px-3"><option value="">Материал...</option>{activeStocks.map((s) => { const p = productMap.get(s.productId); return <option key={s.productId} value={s.productId}>{p?.name}</option>; })}</select><input value={qty} onChange={(e) => setQty(e.target.value)} type="number" step="0.001" min="0" required placeholder="Новый остаток" className="h-11 w-full rounded-xl border px-3"/><button disabled={busy} className="h-11 w-full rounded-xl bg-slate-900 font-black text-white">Сохранить остаток</button></form>}
