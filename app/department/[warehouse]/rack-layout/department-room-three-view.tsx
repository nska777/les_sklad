...
export function DepartmentRoomThreeView({
  ...
  const cameraAction = useCallback((preset: CameraPreset) => {
    ...
    const distance = Math.max(7, camera.position.clone().sub(center).length());
    ...
    if (preset === "top") camera.position.set(center.x, center.y + Math.max(18, distance), center.z + .01);
    if (preset === "zoomIn") camera.position.lerp(center, .18);
    ...
    if (preset === "rotateLeft" || preset === "rotateRight") {
      ...
      camera.position.x = center.x + horizontal.x;
      camera.position.z = center.z + horizontal.z;
    }
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog(0xf1f5f9, 30, 70);
    ...
    camera.position.set(center.x + 13, center.y + 10, center.z + 17);
    camera.lookAt(center);
    ...
    key.position.set(center.x + 10, 16, center.z + 10);
    key.castShadow = true;
    scene.add(key);
    const roomFloor = new THREE.Mesh(new THREE.PlaneGeometry(42, 32), new THREE.MeshStandardMaterial({ color: 0xe5e7eb, roughness: .94 }));
    roomFloor.rotation.x = -Math.PI / 2;
    roomFloor.position.set(center.x, 0, center.z);
    roomFloor.receiveShadow = true;
    scene.add(roomFloor);
    const grid = new THREE.GridHelper(42, 42, 0x94a3b8, 0xcbd5e1);
    grid.position.set(center.x, .01, center.z);
    scene.add(grid);

    const cellInteractive: THREE.Object3D[] = [];
    const rackInteractive: THREE.Object3D[] = [];
    const cellByObject = new Map<string, Cell>();
    const rackByObject = new Map<string, Rack>();
    ...
    rackOnly.forEach((rack, rackIndex) => {
      const group = new THREE.Group();
      const pos = autoRackPosition(rack, rackIndex, false);
      ...
      const rowWidths = rows.map((row) => row.reduce((sum, c) => sum + Math.max(.25, Number(c.cellWidth || Number(rack.width || 4) / Math.max(1, rack.columns))), 0));
      ...
      const shelfT = .09, baseT = .18;
      const height = rowHeights.reduce((sum, h) => sum + h + shelfT, 0) + baseT;

      addBox(group, rackWidth + .4, baseT, rackDepth + .25, 0, baseT / 2, 0, steel);
      let yCursor = baseT;
      rows.forEach((row, rr) => {
        const rowH = rowHeights[rr];
        const totalW = rowWidths[rr] || rackWidth;
        let xCursor = -totalW / 2;
        row.forEach((cell) => {
          const cw = Math.max(.25, Number(cell.cellWidth || rackWidth / Math.max(1, row.length)));
          const ch = Math.max(.25, Number(cell.cellHeight || rowH));
          const cd = Math.max(.25, Number(cell.cellDepth || rackDepth));
          const x = xCursor + cw / 2, y = yCursor + ch / 2, z = cd / 2 + .04;
          xCursor += cw;
          ...
          const mat = new THREE.MeshStandardMaterial({
            ...
            transparent: true,
            opacity: selected ? .92 : occupied ? .56 : .14,
            emissive: selected ? 0xf59e0b : 0x000000,
            emissiveIntensity: selected ? .45 : 0,
          });
          const hit = new THREE.Mesh(new THREE.BoxGeometry(Math.max(.18, cw - .05), Math.max(.18, ch - .06), .12), mat);
          hit.position.set(x, y, z);
          group.add(hit);
          cellInteractive.push(hit);
          cellByObject.set(hit.uuid, cell);
          addBox(group, .05, ch, Math.max(.2, cd), x - cw / 2, y, 0, shelf);
          ss.slice(0, 3).forEach((stock, idx) => {
            const visual = packageObject(stock, Math.min(.8, Math.max(.42, cw * .45)));
            visual.position.set(x + (idx - (Math.min(ss.length, 3) - 1) / 2) * Math.min(.34, cw * .28), yCursor + .06, z + .08);
            group.add(visual);
          });
          const total = ss.reduce((sum, s) => sum + Number(s.quantity), 0), first = ss[0];
          const label = labelSprite(
            occupied ? `${cell.code}\n${first.productName}${ss.length > 1 ? ` +${ss.length - 1}` : ""}\n${total.toLocaleString("ru-RU")} ${first.unit}` : `${cell.code}\n${cw.toFixed(2)}×${ch.toFixed(2)}×${cd.toFixed(2)} м`,
            selected || occupied,
            .72,
          );
          label.position.set(x, y + .15, z + .34);
          label.scale.x = Math.min(2.7, Math.max(1.05, cw * .85));
          group.add(label);
        });
        if (row.length) addBox(group, .05, rowH, rackDepth, totalW / 2, yCursor + rowH / 2, 0, shelf);
        yCursor += rowH;
        addBox(group, rackWidth + .15, shelfT, rackDepth, 0, yCursor + shelfT / 2, 0, shelf);
        yCursor += shelfT;
      });
      addBox(group, rackWidth + .4, baseT, rackDepth + .25, 0, height, 0, steel);
      for (const x of [-rackWidth / 2, rackWidth / 2]) {
        addBox(group, .14, height, .14, x, height / 2, -rackDepth / 2, steel);
        addBox(group, .14, height, .14, x, height / 2, rackDepth / 2, steel);
      }
      const title = labelSprite(`${rack.code} · ${rack.name}`, true, .82);
      title.position.set(0, height + .65, 0);
      group.add(title);

      const rackHit = new THREE.Mesh(
        new THREE.BoxGeometry(rackWidth + .7, height + .4, rackDepth + .7),
        new THREE.MeshBasicMaterial({ transparent: true, opacity: 0, depthWrite: false }),
      );
      rackHit.position.set(0, height / 2, 0);
      group.add(rackHit);
      rackInteractive.push(rackHit);
      rackByObject.set(rackHit.uuid, rack);
      if (rack.id === selectedRackId) {
        const helper = new THREE.BoxHelper(group, 0xf97316);
    ...
    floorsOnly.forEach((zone, i) => {
      const pos = autoRackPosition(zone, i, true);
      const x = pos.x, z = pos.z;
      const w = Math.max(1.4, Number(zone.width || 2)), d = Math.max(1.4, Number(zone.depth || 2));
      const cell = cells.find((c) => c.rackId === zone.id);
      if (!cell) return;
      const ss = stocks.filter((s) => s.cellId === cell.id && Number(s.quantity) > 0);
      const selected = zone.id === selectedRackId;
      const pad = new THREE.Mesh(new THREE.BoxGeometry(w, .12, d), new THREE.MeshStandardMaterial({ color: selected ? 0xf59e0b : 0xfef3c7, transparent: true, opacity: .72, roughness: .8 }));
      pad.position.set(x, .07, z);
      pad.receiveShadow = true;
      scene.add(pad);
      rackInteractive.push(pad);
      rackByObject.set(pad.uuid, zone);
      ...
      ss.forEach((stock, productIndex) => {
        ...
        const groupZ = z + Math.floor(productIndex / cols) * Math.min(1, d / Math.max(1, cols)) - (Math.floor((ss.length - 1) / cols) * .45);
        for (let b = 0; b < quantity; b += 1) {
          const visual = packageObject(stock, .95);
          visual.position.set(groupX + ((b % 3) - 1) * .42, .12, groupZ + (Math.floor(b / 3) - .5) * .42);
          scene.add(visual);
        }
        const label = labelSprite(`${stock.productName}\n${Number(stock.quantity).toLocaleString("ru-RU")} ${stock.unit}`, true, .58);
        label.position.set(groupX, 1.25 + productIndex * .18, groupZ);
        scene.add(label);
      });
      if (!ss.length) {
      ...
      if (selected) {
        const helper = new THREE.BoxHelper(pad, 0xf97316);
        helper.material.depthTest = false;
        helper.renderOrder = 30;
        scene.add(helper);
      }
    ...
    const onUp = (e: PointerEvent) => {
      ...
      const cellHit = raycaster.intersectObjects(cellInteractive, false)[0];
      if (cellHit) {
        const cell = cellByObject.get(cellHit.object.uuid);
        if (cell) { onCellClick(cell); return; }
      }
      const rackHit = raycaster.intersectObjects(rackInteractive, false)[0];
      if (rackHit) {
        const rack = rackByObject.get(rackHit.object.uuid);
        if (rack) { onRackClick(rack); return; }
      }
      onClearSelection();
    };
    ...
    const resize = () => {
      const w = Math.max(320, host.clientWidth), h = Math.max(640, host.clientHeight);
      camera.aspect = w / h;
  ...
  return <div className="relative overflow-hidden rounded-3xl border bg-slate-100 shadow-inner">
    ...
    <div className="absolute left-1/2 top-3 flex max-w-[94%] -translate-x-1/2 flex-wrap justify-center gap-1.5 rounded-2xl border bg-white/90 p-2 shadow-lg backdrop-blur">
      {btn("Обзор", "overview")}{btn("Спереди", "front")}{btn("Слева", "left")}{btn("Справа", "right")}{btn("Сверху", "top")}{btn("−", "zoomOut")}{btn("+", "zoomIn")}{btn("↶", "rotateLeft")}{btn("↷", "rotateRight")}
    </div>
    <div className="pointer-events-none absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full border bg-white/90 px-4 py-2 text-xs font-medium text-slate-600 shadow">Стеллаж — выбрать объект · ячейка — выбрать ячейку · пустое место — снять выделение</div>
  </div>;
}
