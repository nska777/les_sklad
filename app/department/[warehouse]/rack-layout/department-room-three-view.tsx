...
function labelSprite(text: string, accent = false, scale = 1) {
  ...
  sprite.scale.set(3.4*scale,.96*scale,1); return sprite;
}
...
function packageObject(stock:Stock, scale=1) {
  ...
  if(type.includes("бутыл")){
    const body=new THREE.Mesh(new THREE.CylinderGeometry(.12*scale,.14*scale,.42*scale,20),material); body.position.y=.21*scale;
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.055*scale,.07*scale,.13*scale,16),material); neck.position.y=.485*scale; group.add(body,neck);
  } else if(type.includes("канистр")){
    const body=new THREE.Mesh(new THREE.BoxGeometry(.35*scale,.46*scale,.25*scale),material); body.position.y=.23*scale;
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.055*scale,.055*scale,.09*scale,16),material); neck.position.set(.1*scale,.505*scale,0); group.add(body,neck);
  } else if(type.includes("бан")){
    const body=new THREE.Mesh(new THREE.CylinderGeometry(.16*scale,.17*scale,.28*scale,24),material); body.position.y=.14*scale; group.add(body);
  } else if(type.includes("боч")){
    const body=new THREE.Mesh(new THREE.CylinderGeometry(.25*scale,.25*scale,.55*scale,28),material); body.position.y=.275*scale; group.add(body);
  } else {
    const body=new THREE.Mesh(new THREE.CylinderGeometry(.2*scale,.22*scale,.38*scale,24),material); body.position.y=.19*scale; group.add(body);
  }
...
export function DepartmentRoomThreeView({racks,cells,stocks,selectedCellId,selectedRackId,onCellClick}:{racks:Rack[];cells:Cell[];stocks:Stock[];selectedCellId?:string|null;selectedRackId?:string|null;onCellClick:(cell:Cell)=>void;}){
  ...
  useEffect(()=>{
    ...
    const camera=new THREE.PerspectiveCamera(46,1,.1,140);camera.position.set(center.x+13,center.y+10,center.z+17);camera.lookAt(center);cameraRef.current=camera;
    ...
    const floor=new THREE.Mesh(new THREE.PlaneGeometry(42,32),new THREE.MeshStandardMaterial({color:0xe5e7eb,roughness:.94}));floor.rotation.x=-Math.PI/2;floor.position.set(center.x,0,center.z);floor.receiveShadow=true;scene.add(floor);
    ...
    rackOnly.forEach((rack,rackIndex)=>{
      ...
      rows.forEach((row,rr)=>{
        ...
        row.forEach(cell=>{
          ...
          const mat=new THREE.MeshStandardMaterial({color:selected?0xf59e0b:occupied?0xbfdbfe:0xffffff,transparent:true,opacity:selected?.92:occupied?.56:.14,emissive:selected?0xf59e0b:0x000000,emissiveIntensity:selected?.45:0});
          const hit=new THREE.Mesh(new THREE.BoxGeometry(Math.max(.18,cw-.05),Math.max(.18,ch-.06),.12),mat);hit.position.set(x,y,z);group.add(hit);interactive.push(hit);cellByObject.set(hit.uuid,cell);
          addBox(group,.05,ch,Math.max(.2,cd),x-cw/2,y,0,shelf);
          ss.slice(0,3).forEach((stock,idx)=>{const visual=packageObject(stock,Math.min(.8,Math.max(.42,cw*.45)));visual.position.set(x+(idx-(Math.min(ss.length,3)-1)/2)*Math.min(.34,cw*.28),yCursor+.06,z+.08);group.add(visual);});
        ...
        if(row.length){const last=row[row.length-1],lastW=Math.max(.25,Number(last.cellWidth||1));addBox(group,.05,rowH,rackDepth,totalW/2,yCursor+rowH/2,0,shelf);}
    ...
    floorsOnly.forEach((zone,i)=>{
      ...
      const pad=new THREE.Mesh(new THREE.BoxGeometry(w,.12,d),new THREE.MeshStandardMaterial({color:selected?0xf59e0b:0xfef3c7,transparent:true,opacity:.72,roughness:.8}));pad.position.set(x,.07,z);pad.receiveShadow=true;scene.add(pad);interactive.push(pad);cellByObject.set(pad.uuid,cell);
      ...
      ss.forEach((stock,productIndex)=>{
        ...
        for(let b=0;b<quantity;b++){const visual=packageObject(stock,.95);visual.position.set(groupX+((b%3)-1)*.42,.12,groupZ+(Math.floor(b/3)-.5)*.42);scene.add(visual);}
        const label=labelSprite(`${stock.productName}\n${Number(stock.quantity).toLocaleString("ru-RU")} ${stock.unit}`,true,.58);label.position.set(groupX,1.25+productIndex*.18,groupZ);scene.add(label);
      });
