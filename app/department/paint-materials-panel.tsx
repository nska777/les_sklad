...
export function PaintMaterialsPanel({ products, cells, stocks }: { products: Product[]; cells: Cell[]; stocks: Stock[]; warehouseCode?: string }) {
  ...
  const openMove = (product: Product, stock: Stock) => {
    setMove({ product, stock }); setTargetCellId(""); setQuantity(String(stock.quantity)); setInputUnit(normUnit(product.unit));
  };
  ...
  const submitMove = async () => {
    ...
    try {
      ...
      toast.success("Материал перемещён"); setMove(null); window.setTimeout(() => window.location.reload(), 200);
  ...
  return <>
      ...
      <div className="divide-y">
        {filtered.map((p) => {
          ...
          return <article key={p.id} className="grid gap-4 p-4 sm:p-5 lg:grid-cols-[minmax(240px,1.3fr)_280px_160px_minmax(310px,1fr)] lg:items-center">
            <div><div className="font-black leading-5">{p.name}</div><div className="mt-1 text-xs text-slate-500">{p.sku}{p.oneCId ? ` · 1С ${p.oneCId}` : ""}</div><div className="mt-2 flex flex-wrap gap-1.5 text-[11px] font-semibold text-slate-600">{p.category && <span className="rounded-full bg-slate-100 px-2.5 py-1">{p.category}</span>}{p.packType && <span className="rounded-full bg-slate-100 px-2.5 py-1">{p.packType}</span>}{p.ral && <span className="rounded-full bg-slate-100 px-2.5 py-1">RAL {p.ral}</span>}</div>{isAdmin && <Button type="button" size="sm" variant="outline" className="mt-3 text-red-600" disabled={busy} onClick={()=>void deleteProduct(p)}><Trash2 size={14}/> Удалить материал</Button>}</div>
    ...
    {move && <div className="fixed inset-0 z-[50000] flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm" onMouseDown={(e)=>{ if(e.target===e.currentTarget&&!busy)setMove(null); }}><section className="w-full max-w-2xl rounded-[28px] bg-white p-5 shadow-2xl sm:p-6">
