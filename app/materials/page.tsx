...
import { ArrowLeft, ArrowRightLeft, Boxes, Layers3, Loader2, MapPin, Search } from "lucide-react";
    ...
    <div className="mx-auto max-w-[1650px] space-y-4">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Input value={operator} onChange={(event) => setOperator(event.target.value)} className="w-44 bg-white" placeholder="Кладовщик" />
          ...
          {filtered.map((product) => {
            ...
            return <article key={product.id} className="grid gap-4 p-4 sm:p-5 xl:grid-cols-[minmax(260px,1.5fr)_140px_minmax(340px,2fr)] xl:items-start">
              ...
              <div><div className="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">Места хранения</div>{stocks.length ? <div className="grid gap-2 sm:grid-cols-2 2xl:grid-cols-3">{stocks.map((stock) => {
                ...
                if (!cell) return null;
                return <div key={stock.cellId} className="rounded-2xl border border-black/10 bg-white p-3"><div className="flex items-start justify-between gap-2"><div><div className="flex items-center gap-1.5 font-mono font-bold"><MapPin size={14} />{cell.code}</div><div className="mt-1 text-xs text-slate-500">{rack?.name || rack?.code || "Стеллаж"} · {cell.side === "front" ? "лицевая" : "задняя"} · полка {cell.rowIndex + 1}</div></div><b>{qty(stock.quantity)}</b></div><div className="mt-3 grid grid-cols-2 gap-2"><Button asChild size="sm" variant="outline"><Link href={`/rack-layout?cell=${encodeURIComponent(cell.id)}`}><Layers3 /> 3D</Link></Button><Button size="sm" onClick={() => openMove(product, cell.id, stock.quantity)}><ArrowRightLeft /> Переместить</Button></div></div>;
    ...
    <Dialog open={!!move} onOpenChange={(open) => !open && setMove(null)}>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl">
        ...
        <div className="mt-5 space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2"><div><Label>Количество</Label><Input className="mt-2" type="number" min="0.001" max={move.max} step="any" value={amount} onChange={(event) => setAmount(event.target.value)} /></div><div><Label>Кто перемещает</Label><Input className="mt-2" value={operator} onChange={(event) => setOperator(event.target.value)} /></div></div>
        </div>
