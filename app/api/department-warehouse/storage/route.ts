  ...
  try {
    ...
    if (action === "saveVisualLayout") {
      ...
      for (const item of changes) {
        ...
        if (!ownedIds.has(cellId)) continue;
        await upsertDimensions(db, ctx.warehouseCode, cellId, clamp(numberValue(item.width, 1), .2, 8), clamp(numberValue(item.height, 1.1), .2, 6), clamp(numberValue(item.depth, rack.depth), .2, 6));
        updated += 1;
      }
    ...
    if (action === "addVisualCell") {
      ...
      const totalWidth = widths.reduce((s, v) => s + v, 0);
      const proposed = totalWidth / Math.max(1, rowCells.length);
      const scale = totalWidth / (totalWidth + proposed);
      ...
      await upsertDimensions(db, ctx.warehouseCode, id, clamp(proposed * scale, .2, 8), Number(base?.height || 1.1), Number(base?.depth || rack.depth));
    ...
    if (action === "deleteVisualCell") {
      ...
      const rowCells = await db.select().from(departmentCells).where(and(eq(departmentCells.warehouseCode, ctx.warehouseCode), eq(departmentCells.rackId, cell.rackId), eq(departmentCells.rowIndex, cell.rowIndex)));
