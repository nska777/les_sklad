...
import type { WarehouseRole } from "@/lib/warehouse-auth";
...
export async function POST(request: NextRequest) {
  ...
  const operator = request.headers.get("x-warehouse-user") || "Администратор";
  ...
  if (action === "create") {
    ...
    await db.insert(warehouseUsers).values({ id, username, name, passwordHash: hash, passwordSalt: salt, role, active: true });
    await db.insert(activityLogs).values({ id: crypto.randomUUID(), action: "Создан пользователь", entityType: "Пользователь", entityId: id, entityName: name, details: `${username} · ${role}`, operator });
  ...
  if (action === "update") {
    ...
    const values: { name: string; role: string; active: boolean; passwordHash?: string; passwordSalt?: string } = { name, role, active };
    if (password) {
    ...
    await db.insert(activityLogs).values({ id: crypto.randomUUID(), action: "Изменён пользователь", entityType: "Пользователь", entityId: id, entityName: name, details: `${current.username} · ${role} · ${active ? "активен" : "отключён"}`, operator });
