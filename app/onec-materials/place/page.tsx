      ...
      <div className="mx-auto max-w-[1500px] space-y-4">
        ...
        <section className="grid gap-4 xl:grid-cols-[1.05fr_.95fr]">
          ...
          <div className="panel p-4 sm:p-5">
            ...
            {placed ? <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-900"><div className="flex items-center gap-2 font-bold"><CheckCircle2 size={19} /> Размещение сохранено</div><p className="mt-1 text-sm">{placed.cellCode} · {qty(Number(placed.quantity || 0))} шт. Товар уже отображается в 3D-стеллаже.</p><div className="mt-3 flex flex-wrap gap-2"><Button asChild size="sm"><Link href={chosenCell ? `/rack-layout?cell=${encodeURIComponent(chosenCell.id)}` : "/rack-layout"}><Layers3 /> Посмотреть эту ячейку в 3D</Link></Button><Button size="sm" variant="outline" onClick={() => { setPlaced(null); setSelectedItem(null); setQuery(""); setItems([]); setQuantity(""); setCellId(""); }}>Следующий материал</Button></div></div> : <Button disabled={saving || !selectedItem || !chosenCell || Number(quantity) <= 0 || Number(quantity) > remaining} onClick={() => void place()} className="accent-button mt-5 h-12 w-full"><PackagePlus /> {saving ? "Сохраняю..." : "Разместить в выбранную ячейку"}</Button>}
          </div>
        </section>
      </div>
    </main>
