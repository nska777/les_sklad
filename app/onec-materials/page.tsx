        ...
        <div className="divide-y divide-black/10">
          {items.map((item) => {
            ...
            return <article key={item.id} className="grid gap-3 p-3 sm:p-4 xl:grid-cols-[minmax(260px,2fr)_110px_120px_minmax(210px,1.2fr)_minmax(220px,1.1fr)_125px] xl:items-center">
              ...
              <div className="min-w-0"><div className="mb-1 text-xs text-[#7a877f]">Где лежит · нажмите для исправления</div>{item.locations?.length ? <div className="flex flex-wrap gap-1.5">{item.locations.map((location) => <button key={location.cellId} type="button" onClick={() => openCorrection(item, location)} className="inline-flex items-center gap-1.5 rounded-lg bg-[#f3f6f3] px-2 py-1 text-xs transition hover:bg-blue-50 hover:ring-1 hover:ring-blue-200"><MapPin size={12} /><b className="font-mono">{location.cellCode}</b><span>· {qty(location.quantity)}</span><Pencil size={11} /></button>)}</div> : <span className="text-sm text-[#7a877f]">Не размещён</span>}</div>
    ...
    <Dialog open={!!selected} onOpenChange={(open) => !open && setSelected(null)}>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl">
        {selected && <>
          ...
          <DialogFooter className="mt-6">{lastPlacement ? <><Button variant="outline" asChild><Link href="/rack-layout"><Layers3 /> Посмотреть в 3D</Link></Button><Button onClick={() => setSelected(null)} className="accent-button">Закрыть</Button></> : <><Button variant="outline" onClick={() => setSelected(null)}>Отмена</Button><Button disabled={saving || !chosenCell || Number(quantity) <= 0 || Number(quantity) > selectedRemaining} onClick={() => void place()} className="accent-button">{saving ? <Loader2 className="animate-spin" /> : <PackagePlus />} Разместить</Button></>}</DialogFooter>
        </>}
      </DialogContent>
    </Dialog>
