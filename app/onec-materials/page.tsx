...
type ApiResult = { items: Item[]; stats: { total: number; linked: number }; pagination: Pagination; error?: string };
...
export default function OneCMaterialsPage() {
  ...
  const place = async () => {
    ...
    try {
      ...
      toast.success("Материал размещён", { description: `${result.cellCode || chosenCell.code} · ${qty(entered)} шт.` });
  ...
  const saveCorrection = async () => {
    ...
    try {
      ...
      toast.success("Количество исправлено", { description: `${correction.location.cellCode}: ${qty(correction.location.quantity)} → ${qty(next)} шт.` });
      setCorrection(null);
      await Promise.all([loadItems(query, page), loadLayout()]);
    } catch (error) {
  ...
  return <main className="min-h-screen px-3 py-4 text-[var(--foreground)] sm:px-5 lg:px-7">
    <div className="mx-auto max-w-[1650px] space-y-4">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div className="min-w-0">
          ...
          <p className="page-description">Найдите материал и размещайте его по физическому адресу: стеллаж → сторона → полка → ячейка.</p>
        </div>
      ...
      <section className="panel overflow-hidden">
        ...
        <div className="divide-y divide-black/10">
          {items.map((item) => {
            ...
            return <article key={item.id} className="grid gap-3 p-3 sm:p-4 xl:grid-cols-[minmax(260px,2fr)_110px_120px_minmax(210px,1.2fr)_minmax(220px,1.1fr)_125px] xl:items-center">
              <div className="min-w-0"><div className="break-words font-semibold leading-5">{item.name}</div><div className="mt-1 text-xs text-[#7a877f]">Строка Excel: {item.sourceRow}{item.reserved1c ? ` · резерв/вычет: ${qty(item.reserved1c)}` : ""}</div></div>
              <div className="text-sm xl:text-right"><span className="text-xs text-[#7a877f] xl:block">По 1С</span><b>{qty(item.quantity1c)}</b></div>
              <div className="text-sm xl:text-right"><span className="text-xs text-[#7a877f] xl:block">Осталось</span><b className={remaining < 0 ? "text-red-600" : "text-blue-700"}>{qty(Math.max(0, remaining))}</b></div>
              <div className="min-w-0"><div className="mb-1 text-xs text-[#7a877f]">Где лежит · карандаш — исправить, 3D — показать</div>{item.locations?.length ? <div className="flex flex-wrap gap-1.5">{item.locations.map((location) => <div key={location.cellId} className="inline-flex overflow-hidden rounded-lg bg-[#f3f6f3] text-xs ring-1 ring-transparent transition hover:ring-blue-200"><button type="button" onClick={() => openCorrection(item, location)} className="inline-flex items-center gap-1.5 px-2 py-1 hover:bg-blue-50"><MapPin size={12} /><b className="font-mono">{location.cellCode}</b><span>· {qty(location.quantity)}</span><Pencil size={11} /></button><Link href={`/rack-layout?cell=${encodeURIComponent(location.cellId)}`} className="inline-flex items-center border-l border-black/10 px-2 py-1 font-semibold text-blue-700 hover:bg-blue-100" title={`Показать ${location.cellCode} в 3D`}><Layers3 size={12} /></Link></div>)}</div> : <span className="text-sm text-[#7a877f]">Не размещён</span>}</div>
              ...
              <div className="xl:text-right"><Button disabled={complete} onClick={() => openPlacement(item)} className="accent-button w-full xl:w-auto"><PackagePlus /> {complete ? "Размещено" : item.linkedProductId ? "Добавить" : "Разместить"}</Button></div>
            </article>;
          })}
    ...
    <Dialog open={!!selected} onOpenChange={(open) => !open && setSelected(null)}>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl">
        {selected && <>
          <DialogHeader><DialogTitle>{lastPlacement ? "Материал размещён" : "Разместить материал"}</DialogTitle><DialogDescription>{lastPlacement ? "Остаток записан в физическую ячейку." : "Выберите адрес по архитектуре склада: стеллаж → сторона → полка → ячейка."}</DialogDescription></DialogHeader>
          <div className="mt-4 rounded-2xl border border-blue-200 bg-blue-50/70 p-4"><div className="font-bold">{selected.name}</div><div className="mt-2 grid gap-2 text-sm sm:grid-cols-3"><span>Доступно 1С: <b>{qty(selected.available1c)}</b></span><span>Размещено: <b>{qty(selected.placedQuantity)}</b></span><span>Осталось: <b>{qty(selectedRemaining)}</b></span></div>{lastPlacement?.cellCode && <div className="mt-3 flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-sm"><MapPin size={16} /><b>{lastPlacement.cellCode}</b><span>· {qty(Number(lastPlacement.quantity || 0))} шт.</span></div>}</div>
          ...
          {!lastPlacement && <div className="mt-5 space-y-5">
            {layoutLoading ? <div className="flex min-h-52 items-center justify-center"><Loader2 className="animate-spin" /></div> : !racks.length ? <div className="rounded-xl border border-orange-200 bg-orange-50 p-4 text-sm text-orange-800">Сначала создайте хотя бы один стеллаж.</div> : <>
              ...
              <div className="grid gap-4 sm:grid-cols-2"><div><Label>Количество · максимум {qty(selectedRemaining)}</Label><Input type="number" min="0.001" max={selectedRemaining || undefined} step="any" value={quantity} onChange={(event) => setQuantity(event.target.value)} className="mt-2" /></div><div><Label>Кто размещает</Label><Input value={operator} onChange={(event) => setOperator(event.target.value)} className="mt-2" /></div></div>
            </>}
          </div>}

          {lastPlacement && <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900"><div className="flex items-center gap-2 font-bold"><QrCode size={18} /> Готово</div><p className="mt-1">Товар уже отображается в 3D-стеллаже и фактических остатках.</p>{(lastPlacement.internalCode || selected.internalCode) && <div className="mt-3 font-mono font-bold">{lastPlacement.internalCode || selected.internalCode}</div>}</div>}

          <DialogFooter className="mt-6">{lastPlacement ? <><Button variant="outline" asChild><Link href={chosenCell ? `/rack-layout?cell=${encodeURIComponent(chosenCell.id)}` : "/rack-layout"}><Layers3 /> Посмотреть эту ячейку в 3D</Link></Button><Button onClick={() => setSelected(null)} className="accent-button">Закрыть</Button></> : <><Button variant="outline" onClick={() => setSelected(null)}>Отмена</Button><Button disabled={saving || !chosenCell || Number(quantity) <= 0 || Number(quantity) > selectedRemaining} onClick={() => void place()} className="accent-button">{saving ? <Loader2 className="animate-spin" /> : <PackagePlus />} Разместить</Button></>}</DialogFooter>
        </>}
      </DialogContent>
    </Dialog>

    <Dialog open={!!correction} onOpenChange={(open) => !open && setCorrection(null)}><DialogContent className="sm:max-w-lg">{correction && <><DialogHeader><DialogTitle>Исправить размещение</DialogTitle><DialogDescription>История сохранится. Введите фактическое количество и причину.</DialogDescription></DialogHeader><div className="mt-4 rounded-xl bg-slate-50 p-4"><div className="font-semibold">{correction.item.name}</div><div className="mt-1 text-sm text-slate-500">Ячейка {correction.location.cellCode} · сейчас {qty(correction.location.quantity)} шт.</div></div><div className="mt-4 space-y-4"><div><Label>Фактическое количество</Label><Input type="number" min="0" step="any" value={correctQuantity} onChange={(event) => setCorrectQuantity(event.target.value)} className="mt-2" /></div><div><Label>Причина</Label><Input value={correctionReason} onChange={(event) => setCorrectionReason(event.target.value)} placeholder="Например, пересчёт" className="mt-2" /></div><div><Label>Кто исправляет</Label><Input value={operator} onChange={(event) => setOperator(event.target.value)} className="mt-2" /></div></div><DialogFooter className="mt-6"><Button variant="outline" onClick={() => setCorrection(null)}>Отмена</Button><Button disabled={saving || !correctionReason.trim()} onClick={() => void saveCorrection()} className="accent-button">Сохранить</Button></DialogFooter></>}</DialogContent></Dialog>
